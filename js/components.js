...
const switchImageComponent = (switchObj) => {
  const onClick = `toggleSwitchWithAudioAndRendering('${switchObj.name.toLocaleLowerCase()}')`;

  const switchImage = switchObj.state
    ? `<img class="switch-image" src="${SWITCH_ON_IMAGE_URL}" onclick="${onClick}">`
    : `<img class="switch-image" src="${SWITCH_OFF_IMAGE_URL}" onclick="${onClick}">`;
